...
def results():
    ...
    if request.values:
        ...
        if req_type == 'pos':
            inp = inp.upper()
            search_result = db.session.query(Token.token, Sentence.sentence, Sentence.name_of_texts, Sentence.url)\
...
def tri_results():
    if request.values:
        ...
        # базовый запрос
        query = db.session.query(Token1.token, Token2.token, Token3.token, Sentence.sentence, Sentence.name_of_texts, Sentence.url)\
            .join(Token1, Sentence.id_sentence == Token1.id_sentences)\
            .join(Token2, Sentence.id_sentence == Token2.id_sentences) \
        ...
        if req_type1 == 'pos1':
            filters.append(Token1.pos == inp1.upper())
        elif req_type1 == 'lemma1':
            inp_lemms1 = get_lemma(inp1, inp_pos1, translate)
            if inp_lemms1 == 0:
                return render_template('error.html')
            ...
            filters.append(Token1.token == inp1.lower())
            filters.append(Token1.pos.in_(inp_pos1))

        if req_type2 == 'pos2':
            filters.append(Token2.pos == inp2.upper())
        elif req_type2 == 'lemma2':
            inp_lemms2 = get_lemma(inp2, inp_pos2, translate)
            if inp_lemms2 == 0:
                return render_template('error.html')
            ...
            filters.append(Token2.token == inp2.lower())
            filters.append(Token2.pos.in_(inp_pos2))

        if req_type3 == 'pos3':
            filters.append(Token3.pos == inp3.upper())
            ...
            filters.append(Token3.token == inp3.lower())
            filters.append(Token3.pos.in_(inp_pos3))
